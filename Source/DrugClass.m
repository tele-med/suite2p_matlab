classdef DrugClass <handle
    ...
    properties
        ...
        path
        tF %end of baseline
        tD %start of drug application (tF=tD if the baseline is immidiately followed by the drug application)
        tL
        start
        stop
        in
    ...
    methods
        ...
        function allPanelConfig(app,gB)
            ...
            buttonVisualizeExc=uibutton(p2,'Text','E','Position',[350,2,20,20]);
            buttonVisualizeExc.ButtonPushedFcn = @(src,event)visualizeTrace(app,'E');
            
            buttonVisualizeInh=uibutton(p2,'Text','I','Position',[380,2,20,20]);
            buttonVisualizeInh.ButtonPushedFcn = @(src,event)visualizeTrace(app,'I');
            
            buttonVisualizeNoR=uibutton(p2,'Text','NR','Position',[410,2,25,20]);
            buttonVisualizeNoR.ButtonPushedFcn = @(src,event)visualizeTrace(app,'NR');
            
            app.areaDelete=uitextarea(p2,'Value','idx to delete', 'Position', [220,2,100,20]);
           
            %app.areaDelete.ValueChangedFcn= @(areaDelete,event)textEntered(app);
            
          
        end
        
        function textEntered(app)
            %function for deleting cells by entering the suite2p index in
            %the text area field
